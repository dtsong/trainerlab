...
class DeckExportService:
    ...
    async def export_pokemoncard(
        self,
        deck_id: UUID,
        user: User | None = None,
    ) -> str | None:
        """Export a deck to Pokemon Card Live format.
        ...
        Returns:
            Pokemon Card Live formatted string if deck found and accessible,
            None otherwise

        The Pokemon Card Live format:
            Pokemon - 10
            4 Charizard ex SV4 6
            ...
        ...
        card_entries = deck.cards
        if not card_entries:
            return self._build_pokemoncard_empty()
        ...
        for card in cards:
            qty = card_quantities.get(card.id, 1)
            line = self._format_pokemoncard_line(card, qty)
            supertype = card.supertype or ""
        ...
        return self._build_pokemoncard_output(
            pokemon_lines=pokemon_lines,
            trainer_lines=trainer_lines,
            energy_lines=energy_lines,
            pokemon_count=pokemon_count,
    ...
    def _format_pokemoncard_line(self, card: Card, quantity: int) -> str:
        """Format a single card line for Pokemon Card Live export.
    ...
    def _build_pokemoncard_empty(self) -> str:
        """Build Pokemon Card Live output for an empty deck."""
        lines = [
            "Pokemon - 0",
            "",
            "Trainer - 0",
    ...
    def _build_pokemoncard_output(
        ...
        """Build the full Pokemon Card Live output string."""
        lines = []

        # Pokemon section
        lines.append(f"Pokemon - {pokemon_count}")
        lines.extend(pokemon_lines)
        lines.append("")

        # Trainer section
        lines.append(f"Trainer - {trainer_count}")
        lines.extend(trainer_lines)
        lines.append("")
